...
class PDFSmartFiller:
    ...
    def _fill_text(self, kind: str, location: str, value: str) -> int:
        # field-id => direct loose form match first
...
class App(tk.Tk):
    ...
    def _build(self):
        ...
        tk.Entry(top, textvariable=self.pdf_var, width=78).grid(row=0, column=1, sticky="we", padx=6)
        tk.Button(top, text="Browse", command=self._pick_pdf).grid(row=0, column=2)
        tk.Label(top, text="Output PDF").grid(row=1, column=0, sticky="w")
        tk.Entry(top, textvariable=self.out_var, width=78).grid(row=1, column=1, sticky="we", padx=6)
        tk.Button(top, text="Browse", command=self._pick_out).grid(row=1, column=2)
        top.grid_columnconfigure(1, weight=1)

        mapf = tk.LabelFrame(self, text="Mappings", padx=10, pady=10)
        mapf.pack(fill="x", padx=10, pady=(0, 8))
        tk.Label(mapf, text="Type").grid(row=0, column=0)
        tk.OptionMenu(mapf, self.type_var, "auto", "text", "checkbox", "radio").grid(row=0, column=1)
        tk.Label(mapf, text="Location").grid(row=0, column=2, padx=(8, 0))
        tk.Entry(mapf, textvariable=self.loc_var, width=32).grid(row=0, column=3, padx=6)
        tk.Label(mapf, text="Value").grid(row=0, column=4)
        tk.Entry(mapf, textvariable=self.val_var, width=32).grid(row=0, column=5, padx=6)
        tk.Button(mapf, text="Add", command=self._add).grid(row=0, column=6)

        self.listbox = tk.Listbox(mapf, height=8)
        self.listbox.grid(row=1, column=0, columnspan=7, sticky="we", pady=(8, 0))
        self.listbox.bind("<Delete>", lambda _e: self._remove_selected())

        row = tk.Frame(mapf)
        row.grid(row=2, column=0, columnspan=7, sticky="w", pady=(8, 0))
        tk.Button(row, text="Remove Selected", command=self._remove_selected).pack(side="left")
        tk.Button(row, text="Clear", command=self._clear).pack(side="left", padx=6)
        tk.Button(row, text="Fill + Save", command=self._run_fill).pack(side="left", padx=12)
    ...
    def _add(self):
        kind = self.type_var.get().strip().lower() or "auto"
        loc = self.loc_var.get().strip()
        val = self.val_var.get().strip()
        if not loc:
            messagebox.showerror("Missing location", "Enter location.")
            return
        self.pairs.append((kind, loc, val))
        self.listbox.insert("end", f"[{kind}] {loc} -> {val}")
        self.loc_var.set("")
        self.val_var.set("")
    ...
    def _run_fill(self):
        ...
        if not out:
            ...
            return
        if not self.pairs:
            messagebox.showerror("No mappings", "Add at least one mapping.")
            return

        filler = None
        try:
            filler = PDFSmartFiller(inp)
            results = filler.fill_pairs(self.pairs)
            filler.save(out)
            self._refresh_viewers()
            errs = sum(1 for r in results if r.status == "error")
            miss = sum(1 for r in results if r.status == "not found")
            if errs:
                messagebox.showwarning("Done with errors", f"{errs} item(s) failed.")
            elif miss:
                messagebox.showwarning("Done with missing", f"{miss} item(s) not matched.")
            else:
                messagebox.showinfo("Done", "PDF filled successfully.")
        except Exception as exc:
    ...
    def _prefill_paths(self):
        ...
        # Prefer a non-generated PDF as input.
        preferred = sorted(
            pdfs,
            key=lambda p: ("_filled" in os.path.basename(p).lower(), os.path.basename(p).lower()),
        )[0]
        b, e = os.path.splitext(preferred)
        self.pdf_var.set(preferred)
        self.out_var.set(f"{b}_filled{e}")
